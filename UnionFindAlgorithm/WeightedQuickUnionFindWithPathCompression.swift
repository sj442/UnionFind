//
//  WeightedQuickUnionFindWithPathCompression.swift
//  UnionFindAlgorithm
//
...
class WeightedQuickUnionFindWithPathCompression: NSObject {
  ...
  private func root(n: Int) -> Int {
    var x = n
    while x != objects[x] {
      // making every other node in oath point to its grandparent
      objects[x] = objects[objects[x]]
      x = objects[x]
    }
    
    // OR setting the root of each examined node to the root of the tree
    let y = n
    while y != objects[y] {
      objects[y] = x
    }
